	...
	public insert_supplier() {
		...
		btnInsert.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!sup_name.matches("[a-zA-Z ,]+")||sup_name.length() ==0) {
					JOptionPane.showMessageDialog(null, "Supplier name cannot be null or can contain only alphabets");
					count+=1;
				} 
				if (! isValidEmail(sup_email.trim())) {
					JOptionPane.showMessageDialog(null, "Please enter the valid email id");
					count+=1;
				} 
				if (!isValidGSTNo(gst_no.trim())) {
					JOptionPane.showMessageDialog(null, "Please enter valid gst number...");
					count+=1;
				} 
				if (!sup_adr.matches("[a-zA-Z0-9, ]+")||sup_adr.length() ==0) {
					JOptionPane.showMessageDialog(null, "Supplier address cannot be null");
					count+=1;
				} 
				if (!phon.matches("[0-9]+")|| phon.length() !=10) {
					JOptionPane.showMessageDialog(null, "Supplier phone number cannot be null or should be of 10 digits ");
					count+=1;
				}
				...
				if(count==0) {
					...
					try {
						phn = Double.parseDouble(textField_4.getText());
		...
		btnReset.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				insert_supplier insert_supplier = new insert_supplier();
				insert_supplier.setVisible(true);
			}
		});
