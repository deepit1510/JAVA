...
public class Student_view_all {
	 ...
	 */
	public static void student_view_all() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Student_view_all window = new Student_view_all();
	...
	private void initialize() {
		...
		submit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					String sql;
					sql = "select sess from students where admiss_no = " + sess;
					ResultSet rs = stmt.executeQuery(sql);
					if (rs.absolute(1)) {
						Student_search_view search = new Student_search_view(sess);
		...
		cls1.setBounds(269, 108, 115, 20);
		frame.getContentPane().add(cls1);
