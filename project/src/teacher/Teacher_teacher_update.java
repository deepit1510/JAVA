...
public class Teacher_teacher_update {
	 ...
	 */
	public static void teacher_teacher_update() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Teacher_teacher_update window = new Teacher_teacher_update();
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (!t_id.matches("[0-9]+")) {
					...
					try {
						...
						String sql;
						sql = "select t_id from tachers where t_id = " + t_id1;
						ResultSet rs = stmt.executeQuery(sql);
						if (rs.absolute(1)) {
							Teacher_teacher_update_details.teacher_teacher_update_details(t_id);
						} else {
