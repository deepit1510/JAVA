...
public class Student_teacher_update_marks extends JFrame {
	 ...
	 */
	public static void student_teacher_update_marks() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Student_teacher_update_marks frame = new Student_teacher_update_marks(session, cls, admin_no, roll_no, name,
	 ...
	 * Create the frame.
	 */
	public Student_teacher_update_marks(String session, String cls, String admin_no, String roll_no, String name,
		...
		submit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!english.matches("[0-9]+")) {
					JOptionPane.showMessageDialog(null, "INVALID INPUT !!!, Marks must be the Numbers...");
				} else if (!hindi.matches("[0-9]+")) {
					JOptionPane.showMessageDialog(null, "INVALID INPUT !!!, Marks must be the Numbers...");
				} else if (!maths.matches("[0-9]+")) {
					JOptionPane.showMessageDialog(null, "INVALID INPUT !!!, Marks must be the Numbers...");
				} else if (!sst.matches("[0-9]+")) {
					JOptionPane.showMessageDialog(null, "INVALID INPUT !!!, Marks must be the Numbers...");
				} else if (!science.matches("[0-9]+")) {
					JOptionPane.showMessageDialog(null, "INVALID INPUT !!!, Marks must be the Numbers...");
				}

				int ph1 = Integer.parseInt(phn);
				int admiss_no1 = Integer.parseInt(admin_no);
				int cls1 = Integer.parseInt(cls);
				...
				try {
					...
					sql = "update students set session = '" + session + "', class = " + cls1 + ", roll_no = "
							+ roll_no1 + ", name = '" + name + "', dob =  '" + dob + "', gender = '" + gender
							+ "', f_name = '" + fname + "', address = '" + address + "', phn = " + ph1 + ", email = '"
							+ email + "', teacher_id = '" + t_id1 + "', per = " + per + ", remark = '" + remark
							+ "' where admiss_no = " + admiss_no1;
					stmt.executeUpdate(sql);

					sql1 = "update marks set english = " + english1 + ",hindi =  " + hindi1 + ",maths = " + maths1
							+ ",sst = " + sst1 + ",science =  " + science1 + " where admiss_no = " + admiss_no1;
